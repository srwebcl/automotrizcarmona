...
import DypBanner from '@/components/DypBanner';
...
function BrandCard({ name, logo_url }: { name: string; logo_url: string }) {
    ...
    return (
        <Link
            href={`/dyp/cotizar?marca=${encodeURIComponent(name)}`}
            id={`brand-dyp-${slug}`}
...
// ─── Page ──────────────────────────────────────────────────────────────────────
export default async function DypPage() {
    const [allBanners, layoutBrands] = await Promise.all([
        getBanners(),
        getLayoutBrands()
    ]);
    const dypBanners = allBanners
        .filter((b: any) => b.location === 'dyp' && b.active)
                ...
                <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
                    ...
                    <div className="text-center mb-12 md:mb-16">
                        <div className="inline-flex items-center gap-2 mb-4 text-sm font-bold tracking-widest text-gray-500 uppercase">
                            <Wrench size={16} />
                            <span>Desabolladura y Pintura</span>
                        </div>
                        <h1 className="text-3xl sm:text-4xl md:text-5xl font-medium text-gray-900 tracking-tight mb-5">
                            Desabolladura y <span className="font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-gray-700 via-gray-500 to-gray-700">Pintura</span>
                        </h1>
                        <p className="text-lg text-gray-500 font-light max-w-2xl mx-auto leading-relaxed">
                            Selecciona la marca de tu vehículo y completa el formulario para agendar o cotizar.
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4 mb-12">
                        {layoutBrands.cars.filter(b => b.show_in_dyp).map((b) => (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                        {layoutBrands.trucks.filter(b => b.show_in_dyp).map((b) => (
                            <BrandCard key={b.name} name={b.name} logo_url={b.logo_url} />
                        ))}
                    </div>
