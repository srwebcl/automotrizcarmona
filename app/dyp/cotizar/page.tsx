...
function CotizarDypContent() {
    ...
    const ALL_BRANDS = [...layoutBrands.cars, ...layoutBrands.trucks].filter(b => b.show_in_dyp);
    ...
    const brandBranches = React.useMemo(() => {
        if (!form.marca) return branches.filter(b => b.type === 'Desabolladura y Pintura');
        return branches.filter(b => 
            (b.type === 'Desabolladura y Pintura')
        );
    }, [branches, form.marca]);
                    ...
                    <div className="lg:col-span-5 relative">
                        <div className="sticky top-28 bg-white p-6 sm:p-8 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
                            ...
                            <div className="space-y-5 mb-6">
                                {(() => {
                                    const dypBranch = brandBranches.find(b => b.city === form.ciudad && b.type === 'Desabolladura y Pintura')
                                                      || brandBranches.find(b => b.city === form.ciudad) 
                                                      || brandBranches[0] 
                                                      || { address: 'Ruta 5 Norte KM 470, La Serena', city: 'La Serena', email: 'calldyp@carmonaycia.cl', phone: '+56 9 7879 4738' }; // Fallback
                                        ...
                                        <>
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm font-semibold text-gray-900">{dypBranch?.address || 'Ruta 5 Norte KM 470, La Serena'}</p>
                                                    <p className="text-xs text-gray-500 mt-0.5">{dypBranch?.city || 'La Serena'}, Chile</p>
                                                </div>
                                            </div>
                                            <div className="flex items-start gap-4">
                                                <div className="w-10 h-10 rounded-xl bg-[#f8f9fa] flex items-center justify-center flex-shrink-0">
                                                ...
                                                <div>
                                                    ...
                                                    <a href={`mailto:${dypBranch?.email || 'calldyp@carmonaycia.cl'}`}
                                                        className="text-sm font-semibold text-gray-900 hover:text-[#d2001c] transition-colors break-all">
                                                        {dypBranch?.email || 'calldyp@carmonaycia.cl'}
                                                    </a>
                                                </div>
                                            </div>
                                            <div className="border-t border-gray-100 pt-6 space-y-3">
                                                <a href={`tel:${(dypBranch?.phone || '+56 9 7879 4738').replace(/\s+/g, '')}`} id="dyp-phone-btn"
                                                    ...
                                                    <Phone size={18} /> Llamar {dypBranch?.phone || '+56 9 7879 4738'}
                                                </a>
                                            </div>
                                        </>
                                    );
