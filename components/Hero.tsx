                            ...
                            <div key={slide.id} className="relative flex-[0_0_100%] min-w-0 aspect-square">
                                {slide.link ? (
                                    <Link href={slide.link} className="block w-full h-full cursor-pointer relative">
                                        <Image src={slide.image} alt={slide.title} fill sizes="(max-width: 768px) 100vw, 1vw" className="object-cover" priority={slide.id === mobileSlides[0]?.id} fetchPriority={slide.id === mobileSlides[0]?.id ? 'high' : 'auto'} />
                                        {slide.show_text && (
                    ...
                    <div className="flex">
                        {desktopSlides.map((slide) => (
                            <div key={slide.id} className="relative flex-[0_0_100%] min-w-0 aspect-[16/6] md:aspect-[18/7] lg:aspect-[21/8]">
                                {slide.link ? (
                                    <Link href={slide.link} className="block w-full h-full cursor-pointer relative">
                                        <Image src={slide.image} alt={slide.title} fill sizes="(max-width: 768px) 1vw, 100vw" className="object-cover" priority={slide.id === desktopSlides[0]?.id} fetchPriority={slide.id === desktopSlides[0]?.id ? 'high' : 'auto'} />
                                        {slide.show_text && (
                    ...
                    <ul className="flex items-center justify-start md:[&_li]:mx-14 animate-infinite-scroll group-hover:[animation-play-state:paused] w-max flex-shrink-0" aria-hidden="true">
                        {marqueeRow1.map((brand, idx) => (
                            <li key={`2-${idx}`} className="w-[30vw] md:w-auto flex-shrink-0 flex items-center justify-center transition-all duration-300 cursor-pointer opacity-80 hover:opacity-100 hover:scale-110 px-3">
                                <Link tabIndex={-1} href={`${truckBrands.some(t => t.name === brand.name) ? '/camiones' : '/nuevos'}/${brand.slug}`} className={`relative flex items-center justify-center ${brand.logo_url.toLowerCase().includes('iveco') || brand.logo_url.toLowerCase().includes('man') ? 'h-10 md:h-16' : 'h-14 md:h-20'}`}>
                                    <Image
                                        src={brand.logo_url}
                                        alt={brand.name}
                                        width={160}
